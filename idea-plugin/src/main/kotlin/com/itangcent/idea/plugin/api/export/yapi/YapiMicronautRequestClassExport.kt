...
/**
 * Micronaut 环境下的 yapi 参数导出支持类
 * 支持 enableUrlTemplating
 */
@Singleton
@ConditionOnChannel("yapi")
@Exclusion(MicronautRequestClassExporter::class)
open class YapiMicronautRequestClassExporter : MicronautRequestClassExporter(){
        ...
        when {
            ...
            params.contains("!=") -> {
                ...
                if (param == null) {
                    requestBuilderListener.appendDesc(
                        methodExportContext,
                        request, "parameter [$name] should not equal to [$value]"
                    )
                } else {
            ...
            !params.contains('=') -> {
                ...
                if (yapiSettingsHelper.enableUrlTemplating()) {
                    ...
                    param?.let {
                        requestBuilderListener.removeParam(methodExportContext, request, it)
                    }
                } else {
                    if (param == null) {
                        requestBuilderListener.addParam(
